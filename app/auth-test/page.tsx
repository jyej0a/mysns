...
export default function AuthTestPage() {
  ...
  const fetchOrCreateUser = useCallback(async () => {
    ...
    try {
      ...
      // 더 자세한 에러 메시지 생성
      let fullErrorMessage = "사용자 데이터 조회/생성 실패";
      if (err instanceof Error) {
        ...
        if ('code' in err) {
          ...
          if (errorCode === 'PGRST301' || errorCode === '42P01') {
            fullErrorMessage += "\n\n❌ users 테이블이 존재하지 않습니다. Supabase Dashboard에서 마이그레이션을 실행해주세요.";
          } else if (errorCode === '42501') {
            fullErrorMessage += "\n\n❌ 권한이 없습니다. RLS 정책을 확인하거나 개발 환경에서는 RLS를 비활성화해주세요.";
          }
        }
      }
        ...
        <div className="mb-6 p-4 bg-red-50 border border-red-200 rounded-lg flex items-start gap-2">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-red-600 mt-2">
              💡 <strong>해결 방법:</strong>
              <br />
              1. Supabase Dashboard → SQL Editor에서 <code>supabase/migrations/setup_schema.sql</code> 파일을 실행했는지 확인
              <br />
              2. Supabase Dashboard → Table Editor에서 <code>users</code> 테이블이 존재하는지 확인
              <br />
              3. 개발 환경에서는 RLS가 비활성화되어 있는지 확인 (마이그레이션 파일에 <code>DISABLE ROW LEVEL SECURITY</code> 포함)
              <br />
              4. 브라우저 콘솔(F12)에서 더 자세한 에러 로그 확인
              <br />
              5. 환경 변수(.env)가 올바르게 설정되었는지 확인
            </p>
          </div>
          <Button
            variant="ghost"
